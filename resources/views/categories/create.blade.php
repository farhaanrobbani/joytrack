                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" class="mt-1 block w-full border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-lg">
                            ...
                            <option value="expense" {{ old('type', 'expense') === 'expense' ? 'selected' : '' }}>{{ __('Pengeluaran') }}</option>
                        </select>
                        <x-input-error :messages="$errors->get('type')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="icon" :value="__('Icon (opsional)')" />
                        <x-text-input id="icon" name="icon" type="text" class="mt-1 block w-full" :value="old('icon')" placeholder="mis. tag, wallet" />
